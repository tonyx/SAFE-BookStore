...
CREATE TABLE public.aggregate_events_01_wishlist (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_wishlist_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
                                                    aggregate_state_id uuid,
                                                    event_id integer
);
...
CREATE OR REPLACE FUNCTION insert_01_wishlist_event_and_return_id(
    IN event_in TEXT,
...
CREATE OR REPLACE FUNCTION insert_01_wishlist_aggregate_event_and_return_id(
    IN event_in TEXT,
    IN aggregate_id uuid,
    in aggregate_state_id uuid
)
...
CREATE OR REPLACE PROCEDURE set_classic_optimistic_lock_01_wishlist() AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE un_set_classic_optimistic_lock_01_wishlist() AS $$
BEGIN
ALTER TABLE aggregate_events_01_wishlist
